...
const firstCharactersOf = function (strings) {
  return strings.map((str) => str.at(0));
};

// truth values of [0, 1, 2, 3] => [false, true, true, true]
// Assume non-zero numbers are true, and zero is false
const truthValuesOf = function (numbers) {
  return numbers.map((num) => Boolean(num));
};
...
const doubleLettersOf = function (strings) {
  return strings.map((str) => [...str].map((char) => char.repeat(2)).join(""));
};
...
const withoutVowelsOf = function (strings) {
  return strings.map((string) => [...string].map(consonent).join(""));
};
...
const runningSum = function (element) {
  let sum = 0;
  return element.map((num) => (sum += num));
};
